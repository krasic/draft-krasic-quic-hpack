...
appeared earlier on the same connection (refer to HTTP/2 FAQ).  From QUIC's
perspective, this imposes a vulnerability to HoL blocking as described next.
...
avoid HoL blocking, in a backward compatible fashion.

## Basic model

The purpose of QPACK is to allow more flexibility in HPACK, allowing it to avoid
HoL blocking.  QPACK strives to solve HoL blocking in the simplest way
possible. To that end, the mechanisms QPACK defines are largely at the
granularity of header blocks, as opposed to individual header field
representations.
...
2. `HB[i]` can refer to `HA[j]` if `HA` and `HB` are to be delivered in the same
   packet.
3. Otherwise: the encoder chooses between higher compression or avoiding HoL
   blocking.  If avoiding HoL blocking is preferred, an `HB[i]` will be
   represented using and HPACK literal.
...
HPACK indexed entries refer to an entry by it's current position in the dynamic
...
block starts with an integer that conveys its absolute base index, the total
count of entries that had been inserted to the dynamic table.  The format of
individual indexed representations does not change, but their semantics become
absolute in combination with the base index.  Similarly, table insertions use
the base index to avoid ambiguity.

## Changes to HPACK and HTTP over QUIC

QPACK HoL blocking avoidance is optional on a per header block basis, and
signaled by a new flag in the HEADERS and PUSH_PROMISE frames.  If this flag is
absent, then the header block should be processed in strict order as per Section
4.2.1 of the HTTP mapping.

### HPACK changes

QPACK adds three integer *epochs* to HPACK state, all derived from the sequence
numbers of HTTP Mapping (refer to 5.2.2 and 5.2.4.), and provided to the HPACK
layer by the HTTP mapping:

1. *encode epoch*: the sequence number the frame enclosing the header block as
   per the HTTP Mapping.  When entries are added to they dynamic table, the
   current encode epoch is stored with the entry.
2. *packet epoch*: the first encode epoch in the current QUIC packet.  When
   multiple header blocks are packed into a single QUIC packets, the header
...
* then encoing epoch simply increments for every new header sent.

* the mapping layer keeps track of pending encode epochs and monitors transport
  acknolwedgments to determine when the commit epoch can advance, that is when
  the highest in-order acknowledged encode epoch has increased.  *This
  piggybacks on existing QUIC transport mechanisms, no additional wire format
  changes are needed.*
...
not yielded significant gains.  Given this, I think it's best to avoid any
...
It might be advantageous to allow to an implemenation to send header frames on
